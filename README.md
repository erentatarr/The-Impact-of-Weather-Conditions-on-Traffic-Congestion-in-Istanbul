...
**8. Model Comparison and Interpretation**: The performance of the implemented models was evaluated using RMSE and R² metrics. The results show that weather-related variables exhibit a consistent relationship with traffic congestion, but their predictive capacity alone remains limited. Precipitation emerged as the most correlated weather variable with traffic congestion. Overall, these findings confirm that model performance is influenced not only by the chosen methodology but also by the level of detail and information richness of the feature set.[Result Summary – Machine Learning Results](./Result_Summary.docx)
